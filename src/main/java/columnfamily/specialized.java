...
import static columnfamily.derived.len;
import static columnfamily.derived.metalen;
...
public class specialized {
    ...
    public String createForspecialized(Adat a, AdatRelationships [] ar, AdatAttributeRelationships [] adatAtrRel, MultiLevelPan mlp, MultiLevelAdat mla){
        ...
        for(Adat child: children){            
            specialized childClass = new specialized();
          ...
          //   String child_cf = child.getName();
           ...
           //for output
            ...
            output = output + "\n"+ child_o.Cassandra_output(childClass.CassandraList, a, childClass.CassandraMeta, childsize, childmetasize);
            output = output + "\n"+ child_o.HBase_output(childClass.Hbase_col, childClass.Colfamily, childClass.Hbase_meta, child);
            
            if (!(tree.get(child).isEmpty())){
                createForspecialized(child, ar, adatAtrRel, mlp, mla); 
               // childClass.CassandraList.put(childAdatKey, "Bigint");
            }
        }
